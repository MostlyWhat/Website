<script lang="ts">
	...
	import { Blocks, Server, Component, Layout, FileText, Zap } from '@lucide/svelte';

	// Load products overview doc
	const doc = loadDocPage('products', 'overview');
</script>

<svelte:head>
	<title>{doc?.title ?? 'Products'} — Documentation</title>
	<meta name="description" content={doc?.description ?? 'Product documentation for MostlyWhat Systems.'} />
</svelte:head>
...
{#if doc}
	...
	<section class="border-b border-border">
		<div class="px-6 py-12 md:px-12 lg:px-16">
			<span class="font-mono text-[10px] tracking-widest text-muted-foreground">// PRODUCTS</span>
		...
		<div class="border-t border-border px-6 py-8 md:px-12 lg:px-16">
			...
			<h2 class="font-display mt-4 text-2xl font-bold uppercase">DOCUMENTATION</h2>
		</div>
		<div class="grid grid-cols-12 gap-px border-t border-border bg-border">
			{#each [
				{ icon: Blocks, title: 'Architecture', desc: 'System architecture patterns', href: '/docs/products/architecture' },
				{ icon: Component, title: 'SvelteKit Apps', desc: 'Building web applications', href: '/docs/products/sveltekit' },
				{ icon: Server, title: 'Deployment', desc: 'Deploy to Cloudflare & Vercel', href: '/docs/products/deployment' }
			] as item (item.title)}
				<a
