<script lang="ts">
	...
	import { ArrowLeft, Save, Eye, EyeOff, Users, Shield, Globe, Trash2 } from '@lucide/svelte';
	...
	let audience = $state<'user' | 'admin' | 'all'>(data.article?.audience ?? 'user');
	let tags = $state((data.article?.tags ?? []).join(', '));
	let isPublished = $state(data.article?.isPublished ?? false);
...
<svelte:head>
	<title>Edit Article | Knowledge Base | Admin | MostlyWhat Systems</title>
</svelte:head>
...
<div class="min-h-[calc(100dvh-4rem)]">
	...
	<section class="border-b border-border bg-background px-6 py-8 md:px-12">
		<div class="flex items-start justify-between">
			<div>
				<span class="font-mono text-[10px] tracking-widest text-muted-foreground">// EDIT ARTICLE</span>
				<h1 class="font-display mt-2 text-2xl font-bold uppercase md:text-3xl">
					{title || 'Untitled Article'}
				</h1>
				<div class="mt-2 flex items-center gap-2">
					{#if isPublished}
						<span class="inline-flex items-center gap-1 border border-green-500/30 bg-green-500/10 px-2 py-0.5">
							<Eye class="h-3 w-3 text-green-500" />
							<span class="font-mono text-[10px] tracking-wider text-green-500">PUBLISHED</span>
						</span>
					{:else}
						<span class="inline-flex items-center gap-1 border border-yellow-500/30 bg-yellow-500/10 px-2 py-0.5">
							<EyeOff class="h-3 w-3 text-yellow-500" />
							<span class="font-mono text-[10px] tracking-wider text-yellow-500">DRAFT</span>
						</span>
					{/if}
				</div>
			</div>
			...
			<div class="col-span-12 space-y-6 bg-background px-6 py-8 md:px-12 lg:col-span-8">
				...
				<div>
					...
					<textarea
						...
						required
						rows="20"
						class="font-mono mt-2 w-full border border-border bg-card px-4 py-3 text-sm placeholder:text-muted-foreground focus:border-primary focus:outline-none resize-y"
