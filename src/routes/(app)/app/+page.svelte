<script lang="ts">
	...
	const streamedData = $derived((data as any).streamed?.dashboardData as Promise<{
		stats: { activeProjects: number; pendingProposals: number; unpaidInvoices: number; openTickets: number };
		recentActivity: Array<{ type: string; title: string; time: Date | null; icon: string }>;
	}>);
...
<div class="min-h-[calc(100dvh-4rem)]">
	...
	{#await streamedData}
		...
		<!-- Activity & Actions Grid Skeleton -->
		<div class="grid grid-cols-12 gap-px bg-border">
			<div class="col-span-12 bg-background lg:col-span-6">
				...
				<div class="space-y-4 px-6 py-8 md:px-12 lg:px-16">
					{#each Array(4) as _}
	...
	{:then dashboardData}
		{@const stats = [
			{ label: 'ACTIVE PROJECTS', value: String(dashboardData.stats?.activeProjects ?? 0).padStart(2, '0'), icon: FolderKanban, href: '/app/projects' },
			{ label: 'PENDING PROPOSALS', value: String(dashboardData.stats?.pendingProposals ?? 0).padStart(2, '0'), icon: FileText, href: '/app/proposals' },
			{ label: 'UNPAID INVOICES', value: String(dashboardData.stats?.unpaidInvoices ?? 0).padStart(2, '0'), icon: Receipt, href: '/app/invoices' },
			{ label: 'OPEN TICKETS', value: String(dashboardData.stats?.openTickets ?? 0).padStart(2, '0'), icon: Ticket, href: '/app/tickets' }
		]}
		...
		<section class="border-b border-border px-6 py-12 md:px-12 lg:px-16">
			<div class="flex flex-col items-center justify-center text-center">
				<div class="flex h-16 w-16 items-center justify-center border border-destructive/30 bg-destructive/10">
					<Clock class="h-6 w-6 text-destructive" />
				</div>
