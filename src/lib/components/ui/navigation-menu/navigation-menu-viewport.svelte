...
<div class={cn('absolute left-0 top-full flex justify-center')}>
	<NavigationMenuPrimitive.Viewport
		...
		bind:ref
		class={cn(
			'origin-top-center relative mt-1.5 h-(--bits-navigation-menu-viewport-height) w-full overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-90 md:w-(--bits-navigation-menu-viewport-width)',
			className
		)}
	>
		{@render children?.()}
