...
const INTERNALS_FOR = (ref) => INTERNALS$3.has(ref) ? INTERNALS$3.get(ref) : INTERNALS$3.set(ref, {}).get(ref);
...
const exclusionsForDocument = ['Document', 'HTMLDocument'];
const exclusionsForNode = ['Node', 'DocumentFragment', 'ShadowRoot', ...exclusionsForDocument, ...exclusionsForElement];
const exclusionsForEventTarget = ['Event', 'CustomEvent', 'EventTarget', 'AbortSignal', 'MediaQueryList', 'Window', ...exclusionsForNode];
const exclusionsForEvent = ['Event', 'CustomEvent', 'EventTarget', 'AbortSignal', 'MediaQueryList', 'Window', ...exclusionsForNode];
const exclusions = {
...
const polyfill = (target, options) => {
    ...
    for (const name of Object.keys(webAPIs)) {
        ...
        if (!Reflect.has(inheritence, name))
            continue;
...
export { AbortController, AbortSignal, Blob, ByteLengthQueuingStrategy, CSSStyleSheet, CountQueuingStrategy, CustomElementRegistry, CustomEvent, DOMException, Document, DocumentFragment, Element, Event, EventTarget, File, FormData, HTMLDivElement, HTMLDocument, HTMLElement, HTMLHeadElement, HTMLHtmlElement, HTMLImageElement, HTMLStyleElement, HTMLTemplateElement, HTMLUnknownElement, Headers, Image, IntersectionObserver, MediaQueryList, MutationObserver, Node, ReadableByteStreamController, ReadableStream, ReadableStreamBYOBReader, ReadableStreamBYOBRequest, ReadableStreamDefaultController, ReadableStreamDefaultReader, Request, Response, ShadowRoot, StyleSheet, TransformStream, Window, WritableStream, WritableStreamDefaultController, WritableStreamDefaultWriter, cancelAnimationFrame, cancelIdleCallback, clearTimeout, fetch, polyfill, requestAnimationFrame, requestIdleCallback, setTimeout };
